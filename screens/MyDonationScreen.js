      ...
      .then((snapshot) => {
        snapshot.forEach((doc) => {
          ...
          if (requestStatus === "pet Sent") {
            ...
            message =
              this.state.donorName + " has shown interest in donating the pet";
          }
      ...
      rightElement={
        <TouchableOpacity
          style={[
            ...
            {
              backgroundColor:
                item.request_status === "pet Sent" ? "green" : "#ff5722",
            },
          ]}
          onPress={() => {
            this.sendpet(item);
        ...
        <View style={{ flex: 1 }}>
          {this.state.allDonations.length === 0 ? (
            <View style={styles.subtitle}>
              <Text style={{ fontSize: 20 }}>List of all Pet Donations</Text>
            </View>
          ) : (
            <FlatList
